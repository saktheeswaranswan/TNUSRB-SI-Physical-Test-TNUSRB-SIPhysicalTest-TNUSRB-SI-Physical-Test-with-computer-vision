    ...
    const std::vector<float>& confidences, const std::vector<int>& classIds)
{
    ...
    for (size_t i = 0; i < detections.size(); ++i)
    {
        ...
        if (confidence > 0.5)
        {
            ...
            if (classNamesFile[classId] == "cat")
            {
                catDetected = true;
                break;
            }
            else if (classNamesFile[classId] == "person")
            {
                ...
                // Save cropped image
                std::stringstream ss;
                ss << "person_detection_" << std::setfill('0') << std::setw(4) << i << ".jpg";
    ...
    if (!catDetected)
    {
        std::cout << "Cat not detected. Person cropped." << std::endl;
    }
}
